...
namespace Server.Mobiles
{
	...
	public class MasterOfTheArts : BaseCreature //make it use the creature files/settings
	{
		...
		{
			this.Name = "The Master Crafter"; //the name players will see
			this.Body = 185; //how it look like in game
			this.BaseSoundID = 42; //what sound he makes (I still have problems with sound :(

			this.ChampionSpawn = i_ChampionSpawn; //set if it spawn with champ spawn or normal spawn

			this.SetStr( 401, 420 ); //set stats
			this.SetDex( 81, 90 );
			this.SetInt( 201, 220 );

			this.SetHits( 600, 900 ); //set hp

			this.SetDamage( 20, 35 ); //set how much damage ~

			this.SetDamageType( ResistanceType.Physical, 50 ); //which damage type it does
			this.SetDamageType( ResistanceType.Poison, 50 );

			this.SetResistance( ResistanceType.Physical, 60, 70 ); //what resists it have
			...
			this.SetSkill( SkillName.Macing, 110.0 );

			this.Fame = 30000; //its fame/karma
			this.Karma = -30000;
			...
			AddItem( new Gold( 800, 2000 ));
			if ( Utility.Random(25) == 1 )
				AddItem( new PersonalStatueDeed() );

			new Mule().Rider = this; //make it ride on mule
		}
